...
public class TimerUtil
{
    ...
    public static TimerUtil Register(float duration, Action onComplete, Action<float> onUpdate = null,
        bool isLooped = false, bool useRealTime = false, MonoBehaviour autoDestroyOwner = null)
    {
        // create a manager object to update all the timers if one does not already exist.
        if (TimerUtil._manager == null)
        {
            TimerManager managerInScene = Object.FindObjectOfType<TimerManager>();
            if (managerInScene != null)
            {
                TimerUtil._manager = managerInScene;
            }
            else
            {
                GameObject managerObject = new GameObject { name = "TimerManager" };
                TimerUtil._manager = managerObject.AddComponent<TimerManager>();
            }
        }

        TimerUtil timerUtil = new TimerUtil(duration, onComplete, onUpdate, isLooped, useRealTime, autoDestroyOwner);
        TimerUtil._manager.RegisterTimer(timerUtil);
        return timerUtil;
    }
    ...
    /// </summary>
    /// <param name="timerUtil">The timer to cancel.</param>
    public static void Cancel(TimerUtil timerUtil)
    {
        if (timerUtil != null)
        {
            timerUtil.Cancel();
    ...
    /// a <see cref="NullReferenceException"/> if the timer is null.
    /// </summary>
    /// <param name="timerUtil">The timer to pause.</param>
    public static void Pause(TimerUtil timerUtil)
    {
        if (timerUtil != null)
        {
            timerUtil.Pause();
    ...
    /// a <see cref="NullReferenceException"/> if the timer is null.
    /// </summary>
    /// <param name="timerUtil">The timer to resume.</param>
    public static void Resume(TimerUtil timerUtil)
    {
        if (timerUtil != null)
        {
            timerUtil.Resume();
    ...
    public static void CancelAllRegisteredTimers()
    {
        if (TimerUtil._manager != null)
        {
            TimerUtil._manager.CancelAllTimers();
        }
    ...
    public static void PauseAllRegisteredTimers()
    {
        if (TimerUtil._manager != null)
        {
            TimerUtil._manager.PauseAllTimers();
        }
    ...
    public static void ResumeAllRegisteredTimers()
    {
        if (TimerUtil._manager != null)
        {
            TimerUtil._manager.ResumeAllTimers();
        }
    ...
    private TimerUtil(float duration, Action onComplete, Action<float> onUpdate,
    ...
    /// <summary>
    /// Manages updating all the <see cref="TimerUtil"/>s that are running in the application.
    /// This will be instantiated the first time you create a timer -- you do not need to add it into the
    /// scene manually.
    /// </summary>
    private class TimerManager : MonoBehaviour
    {
        private List<TimerUtil> _timers = new List<TimerUtil>();

        // buffer adding timers so we don't edit a collection during iteration
        private List<TimerUtil> _timersToAdd = new List<TimerUtil>();

        public void RegisterTimer(TimerUtil timerUtil)
        {
            this._timersToAdd.Add(timerUtil);
        }

        public void CancelAllTimers()
        {
            foreach (TimerUtil timer in this._timers)
            {
                timer.Cancel();
            }

            this._timers = new List<TimerUtil>();
            this._timersToAdd = new List<TimerUtil>();
        }
